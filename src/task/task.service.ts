...
export class TaskService {
  ...
  async createTask(boardUuid: string, dto: TaskDto, req: Request) {
    ...
    if (
      (await this.userValidations.isNotGuestMember(userUuid)).valueOf() ||
  ...
  async setTaskComment(taskUuid: string, msg: string, req: Request) {
    if (!req['project_access'].isOwner && !req['project_access'].hasAccess) {
      throw new ForbiddenException('Access denied');
    }
    ...
    if (
      ...
      const taskComment = await this.prisma.taskComments.create({
        data: {
          taksUuid: taskUuid,
          userUuid: userUuid,
          message: msg,
      ...
      throw new ForbiddenException(
        "You don't have permissions to comment a task.",
      );
    }
  }

  async getTaskComments(taskUuid: string, req: Request) {
    if (!req['project_access'].isOwner && !req['project_access'].hasAccess) {
      throw new ForbiddenException('Access denied');
    }
    ...
    if (
      ...
      const taskComments = await this.prisma.taskComments.findMany({
        where: {
          taksUuid: taskUuid,
        },
        select: {
