...
export class TaskController {
    ...
    @ApiCreatedResponse({description: 'task created with success', type: TaskDto})
    @ApiForbiddenResponse({description: 'user does not have permissions to create a task'})
    async createTask(
    ...
    @ApiForbiddenResponse({description: 'user does not have permissions to get all tasks'})
    async getTasks(
        @Param('board_uuid') boardUuid: string,
        @Req() req,
    ) {
    ...
    @ApiForbiddenResponse({description: 'user does not have permissions to delete a task'})
    async deleteTask(
    ...
    @ApiForbiddenResponse({description: 'user does not have permissions to update a task'})
    async updateTask(
        @Param('task_uuid') taskUuid: string,
        @Body() dto: TaskDto,
        @Req() req,
    ...
    @ApiCreatedResponse({description: 'user associated with success', type: AssociateToTaskDto})
    @ApiForbiddenResponse({description: 'user does not have permissions to create a task'})
    ...
    @ApiForbiddenResponse({description: 'user does not have permissions to comment a task'})
